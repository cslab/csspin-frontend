...
be created by using the ``csspin_ce.mkinstance`` plugin (see `create new
instance`_).
...
``csspin-workflows`` plugin-package and activating the ``stdworkflows`` plugin (see
`csspin_workflows.stdworkflows`_).
